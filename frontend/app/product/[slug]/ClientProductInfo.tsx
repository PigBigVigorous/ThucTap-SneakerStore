...
export default function ClientProductInfo({ product }: { product: any }) {
  ...
  // Quản lý Carousel ảnh
  const [mainImageIndex, setMainImageIndex] = useState(0);
  const [showFavModal, setShowFavModal] = useState(false);
  
  const isFav = isFavorite(product?.id);

  // Khởi tạo dữ liệu mặc định dựa trên variant đầu tiên
  useEffect(() => {
  ...
  // Xử lý khi đổi màu sắc
  const handleColorChange = (color: any) => {
    setSelectedColor(color);
    setSelectedSize(null);
    setMainImageIndex(0); // Reset về ảnh đầu tiên của màu mới
  };

  // Mảng ảnh hiện tại (tự động lọc theo màu đang chọn)
  const currentGalleryImages = product?.images 
    ? product.images.filter((img: any) => img.color_id === selectedColor?.id).map((img: any) => img.image_url)
    : [product?.base_image_url || '/placeholder.png'];

  // Gom nhóm các màu sắc độc nhất để hiển thị ở bộ chọn màu
  const uniqueColors = product?.variants?.reduce((acc: any[], current: any) => {
    if (!acc.find((item: any) => item.id === current.color.id)) {
      return acc.concat([current.color]);
    }
    return acc;
  }, []) || [];

  // Lọc size khả dụng theo màu đang chọn và sắp xếp từ nhỏ đến lớn
  const availableVariants = product?.variants
    ?.filter((v: any) => v.color.id === selectedColor?.id)
    .sort((a: any, b: any) => {
      const sizeA = parseFloat(a.size.name.replace(/[^\d.-]/g, ''));
      const sizeB = parseFloat(b.size.name.replace(/[^\d.-]/g, ''));
      return sizeA - sizeB;
    }) || [];
  ...
  const prevImage = () => {
    setMainImageIndex((prev) => (prev - 1 + currentGalleryImages.length) % currentGalleryImages.length);
  };
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      ...
      image: currentGalleryImages[0] || product.base_image_url || '/placeholder.png',
  ...
  if (!selectedColor || currentGalleryImages.length === 0) {
    return <div className="min-h-[60vh] flex items-center justify-center">Đang tải...</div>;
  }

  return (
    // BỐ CỤC GRID 12 CỘT CHUẨN NIKE
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 bg-white">
      
      {/* 🌟🌟🌟 CỘT TRÁI: GALLERY ẢNH (Đã làm đẹp chuẩn Nike) 🌟🌟🌟 */}
      <div className="lg:col-span-7 lg:pr-8 pt-10 lg:pt-0">
        <div className="flex flex-row gap-4 sticky top-24">
          
          {/* 1. Cột Thumbnails dọc */}
          <div className="w-[64px] shrink-0 flex flex-col gap-2.5 max-h-[80vh] overflow-y-auto [&::-webkit-scrollbar]:hidden pr-1 pb-4">
            {currentGalleryImages.map((img: string, idx: number) => (
              <button 
                key={idx} 
                onMouseEnter={() => setMainImageIndex(idx)}
                onClick={() => setMainImageIndex(idx)}
                className={`relative aspect-square rounded-md overflow-hidden bg-[#F6F6F6] transition-all duration-200 shrink-0 border-2
                  ${idx === mainImageIndex ? 'border-gray-900' : 'border-transparent hover:border-gray-300'}`}
              >
                <img src={img || '/placeholder.png'} alt={`Góc ${idx + 1}`} className="w-full h-full object-contain mix-blend-multiply p-1" />
              </button>
            ))}
          </div>

          {/* 2. Hero Image Container (Sử dụng aspect-[4/5] để co giãn hoàn hảo) */}
          <div className="flex-1 bg-[#F6F6F6] rounded-xl relative aspect-[4/5] flex items-center justify-center overflow-hidden group">
            <img 
              src={currentGalleryImages[mainImageIndex] || '/placeholder.png'} 
              alt={product.name} 
              className="w-full h-full object-contain mix-blend-multiply transition-opacity duration-300 ease-in-out p-4 sm:p-6" 
            />
            
            {/* 3. Nút điều hướng Carousel (Nằm ở GÓC DƯỚI BÊN PHẢI giống Nike) */}
            <div className="absolute bottom-6 right-6 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
              <button 
                onClick={prevImage}
                className="bg-white p-3 rounded-full shadow-md hover:bg-gray-100 transition-colors flex items-center justify-center"
              >
      ...
      <div className="lg:col-span-5 pt-10 lg:pt-0 lg:px-10 pb-20">
        <div className="sticky top-28 space-y-8">
          ...
          {/* Title & Price */}
          <div className="pb-4">
            <h1 className="text-[32px] font-medium text-gray-900 leading-tight tracking-tight">{product.name}</h1>
          ...
          {uniqueColors.length > 1 && (
            <div className="py-2">
              <p className="font-medium text-gray-900 text-base mb-3">
                Màu sắc: <span className="text-gray-500">{selectedColor.name}</span>
              </p>
          ...
          <div className="py-2">
            <div className="flex justify-between items-center mb-3">
              ...
              <Link 
                href="/size-guide" 
                target="_blank" 
                className="text-gray-500 hover:text-black font-bold text-sm flex items-center gap-1.5 transition-colors uppercase tracking-widest underline underline-offset-4"
              >
          ...
          <div className="space-y-3.5 pt-4">
            <button 
              ...
              className="w-full bg-black text-white hover:bg-gray-800 font-medium py-5 rounded-full transition-colors text-lg active:scale-[0.98]"
            >
              Thêm vào Giỏ hàng
            </button>
            <button 
              onClick={handleToggleFavorite}
              className="w-full bg-white text-gray-900 border border-gray-300 hover:border-gray-900 font-medium py-5 rounded-full transition-colors flex justify-center items-center gap-2.5 text-lg active:scale-[0.98]"
            >
              {isFav ? 'Đã yêu thích' : 'Yêu thích'} 
              <Heart size={20} className={isFav ? "fill-black text-black" : "text-gray-900"} />
            </button>
          </div>
          
          {/* Description & Details */}
          <div className="pt-10 pb-6 border-t border-gray-100 mt-10">
            <p className="text-gray-900 text-base leading-relaxed font-medium mb-6">
              {product.description || 'Chưa có mô tả cho sản phẩm này.'}
            </p>
            <ul className="space-y-2.5 text-base text-gray-900 font-medium list-disc pl-5 marker:text-gray-900">
              <li>Màu sắc hiển thị: <span className="text-gray-600">{selectedColor.name}</span></li>
          ...
          <div className="border-t border-gray-100 divide-y divide-gray-100">
            <details className="group [&_summary::-webkit-details-marker]:hidden">
              ...
              <div className="pb-8 text-gray-700 text-base font-medium space-y-4 pr-4 leading-relaxed animate-in fade-in slide-in-from-top-1">
                ...
                <p>Thành viên của chúng tôi được tận hưởng đặc quyền hoàn trả miễn phí trong 30 ngày.</p>
              </div>
            </details>
          </div>
      ...
      >
        ...
        <div className="flex gap-4 mb-7">
          <div className="w-[90px] h-[90px] bg-[#F6F6F6] rounded-xl overflow-hidden shrink-0">
            <img src={currentGalleryImages[0] || '/placeholder.png'} alt={product.name} className="w-full h-full object-contain mix-blend-multiply p-2" />
          </div>
          <div className="flex-1 text-sm font-medium">
            <p className="text-gray-900 leading-tight text-base">{product.name}</p>
            <p className="text-gray-500 mt-1">Giày Nam</p>
            <p className="text-gray-900 mt-2.5 font-bold text-base">
                {Number(selectedVariant ? selectedVariant.price : product.variants[0]?.price).toLocaleString('vi-VN')} ₫
            </p>
          </div>
        </div>
        <Link 
          ...
          className="w-full block text-center bg-white text-gray-900 border border-gray-300 hover:border-gray-900 font-bold py-4 rounded-full transition-colors text-base"
        >
