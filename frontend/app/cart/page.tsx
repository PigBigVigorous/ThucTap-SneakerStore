...
export default function CartPage() {
  ...
  const freeshipThreshold = 5000000; // Mốc freeship 5 củ
      ...
      <div className="max-w-[1100px] mx-auto px-4 sm:px-6 pt-10">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          ...
          <div className="w-full lg:w-[65%]">
            ...
            {/* Thanh tiến trình FOMO Freeship */}
            <div className="bg-[#F5F5F5] p-5 rounded-2xl mb-8">
              ...
              <div className="w-full bg-gray-200 h-2 mt-4 rounded-full overflow-hidden">
                <div 
                  className="bg-[#FA5400] h-full transition-all duration-700 ease-out rounded-full"
            ...
            <div className="space-y-8 divide-y divide-gray-100 border-b border-gray-100 pb-8">
              {cart.map((item) => (
                <div key={item.variant_id} className="pt-8 first:pt-0 flex gap-4 sm:gap-6">
                  ...
                  {/* Ảnh sản phẩm vuông vức */}
                  <Link href={`/product/${item.product_name.toLowerCase().replace(/ /g, '-')}`} className="w-[150px] h-[150px] shrink-0 bg-[#F5F5F5] rounded-xl overflow-hidden block">
                    <img 
                      src={item.image} 
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-medium text-gray-900 text-base">{item.product_name}</h3>
                        <p className="text-gray-500 text-base mt-1">Giày Nam</p>
                        <p className="text-gray-500 text-base mt-0.5">{item.color_name}</p>
                        
                        <div className="flex items-center gap-6 mt-3">
                          <p className="text-gray-500 text-base">Size: <span className="text-gray-900">{item.size_name.replace('EU-', '')}</span></p>
              ...
              <details className="group [&_summary::-webkit-details-marker]:hidden mb-6 border-b border-gray-100 pb-2">
                ...
                <div className="pt-4 pb-6 flex gap-2">
                  <input 
                    type="text" 
                    placeholder="Nhập mã" 
                    className="flex-1 border border-gray-300 rounded-lg px-4 py-3 outline-none focus:border-black transition-colors font-medium"
                  />
                  <button className="bg-white text-gray-900 border border-gray-300 hover:border-black font-medium px-6 py-3 rounded-lg transition-colors">
                    Áp dụng
                  </button>
                </div>
              </details>
              ...
              <div className="space-y-4 text-base font-medium border-b border-gray-100 pb-6 mb-6">
                ...
                <div className="flex justify-between items-start text-gray-900">
                  <div className="flex items-center gap-1.5 cursor-pointer hover:underline">
                    Giao hàng & Xử lý <HelpCircle size={14} className="text-gray-500"/>
                  </div>
