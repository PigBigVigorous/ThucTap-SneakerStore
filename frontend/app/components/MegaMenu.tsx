...
export default function MegaMenu() {
  ...
  // 🚨 Ẩn toàn bộ MegaMenu (danh mục shopping) nếu là trang Quản trị
  ...
  return (
    ...
    >
      ...
      <div
        className={`fixed inset-0 top-16 bg-black/40 backdrop-blur-sm transition-opacity duration-300 z-40 ${activeMenu ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
      />

      {/* 3. Bảng Mega Menu Thả Xuống Tràn Viền */}
      <div
        className={`absolute left-0 top-full w-full bg-white z-50 transition-all duration-300 ease-out border-t border-gray-100 shadow-xl overflow-hidden ${activeMenu ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
          }`}
      >
        <div className="max-w-6xl mx-auto px-8 py-10">
          {MENU_DATA.map((menu) => (
            <div
              key={menu.id}
              className={`flex justify-center gap-16 transition-opacity duration-300 absolute inset-x-0 py-10 top-0 ${activeMenu === menu.id ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'
                }`}
            >
