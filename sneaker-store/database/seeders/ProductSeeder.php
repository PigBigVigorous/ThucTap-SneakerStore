...
class ProductSeeder extends Seeder
{
    ...
    {
        // Require MasterData to be seeded first
        $nike = Brand::where('name', 'Nike')->first();
        $catSneakerNam = Category::where('name', 'Sneaker Nam')->first();
        $mainBranch = Branch::first();
        $colors = Color::take(2)->get();
        $sizes = Size::skip(1)->take(2)->get(); // 40, 41
        
        if(!$nike || !$catSneakerNam || !$mainBranch) {
            return;
        }
