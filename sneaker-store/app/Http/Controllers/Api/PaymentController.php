        ...
        if ($secureHash == $vnp_SecureHash) {
            ...
            if ($order) {
                if ($request->vnp_ResponseCode == '00') {
                    ...
                    return response()->json(['success' => false, 'message' => 'Thanh toán thất bại hoặc đã bị hủy']);
                }
            }
            return response()->json(['success' => false, 'message' => 'Không tìm thấy đơn hàng']);
        } else {
            return response()->json(['success' => false, 'message' => 'Chữ ký không hợp lệ (Invalid Signature)']);
        }
    }
